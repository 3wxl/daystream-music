<template>
  <el-container class="songs-container">
    <el-aside
      width="260px"
      class="shadow-md border-[#e4e7ed] bg-white rounded-[10px] p-[10px] overflow-hidden"
    >
      <el-scrollbar height="87vh" class="relative">
        <AdminInput
          type="text"
          placeholder="请输入歌手名"
          v-model="musicianSearch"
          width="w-[220px]"
          lable=""
          rightIcon="sousuo"
          class="flex ml-2 absolute top-0"
        ></AdminInput>
        <ul class="mt-[50px] flex flex-col items-center">
          <li
            @click="activeId = musicianObj.id"
            :class="
              activeId === musicianObj.id ? 'bg-[#529FFD] text-[white]' : 'hover:bg-[#d1e5fa]'
            "
            class="min-w-[215px] px-[10px] py-[6px] my-0.5 rounded-[6px] cursor-pointer z-7 duration-300"
            v-for="musicianObj in musicianList"
            :key="musicianObj.id"
          >
            <IconFontSymbol
              name="yinleren"
              size="16px"
              class="ml-2 mr-3"
              :color="activeId === musicianObj.id ? 'white' : '#666'"
            ></IconFontSymbol>
    ...
    <el-main style="padding: 0 10px 0px 8px">
      <div class="shadow-md border-[#e4e7ed] bg-white rounded-[10px] p-[15px] flex items-center">
        ...
        <AdminSelect
          ...
          label="类型"
        ></AdminSelect>
        <AdminButton text="搜索" class="ml-4 text-[15px]!" @click="handleSearch" />
      </div>
        ...
        <div class="flex justify-between">
          ...
          <div class="flex mr-4">
            ...
            <el-tooltip content="删除所选歌曲">
              <IconFontSymbol
                name="shanchu"
                class="font-700 relative top-[3px] cursor-pointer hover:text-red-700 mr-4"
              ></IconFontSymbol>
            </el-tooltip>
          ...
          <el-table :data="songsList" stripe>
            ...
            <el-table-column label="歌名" width="180" align="center" class="relative">
              <template #default="scope">
                ...
                <span
                  class="absolute left-[77px] text-[10px]/[16px] font-[600] rounded-[4px] px-[4px] py-[0px] bg-[#3278EA] text-[white]"
                  >原创</span
                >
              </template>
            </el-table-column>
            ...
            <el-table-column label="vip" width="60" align="center">
              <template #default="scope">
                <span
                  class="px-[3px] border-[1.5px] rounded-[6px] border-[#3278EA] text-[13px] text-[#3278EA]"
                  >vip</span
                >
            ...
            <el-table-column label="分类" width="150" align="center">
              <template #default="scope">
                <span
                  v-for="(type, index) in scope.row.type"
                  :key="index"
                  class="text-[14px] px-[8px] py-[3px] rounded-[8px] bg-[#EDF3F9] mx-1 text-[#529FFD]"
                  >{{ type }}</span
                >
              </template>
            </el-table-column>
