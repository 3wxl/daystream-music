    ...
    <div class="ml-5 flex items-start">
          ...
          <div class="mt-4 border-t-[rgba(255,255,255,0.1)] border-t-[1px] pt-5">      <!-- 评论/数据详情 -->
            <div class="flex justify-center gap-40">
              <span v-if="!dynamic.isLike" @click="throttleLikeDynamic(dynamic.postId)" class="text-[white] px-[15px] py-[2px] rounded-[15px] cursor-pointer bg-pink-500/20 text-pink-200 shadow-lg border border-pink-400/60" title="点赞">
                ...
                <span class="text-white">{{ dynamic.likeCount }}</span>
              </span>
              <span v-if="dynamic.isLike" @click="throttleLikeDynamic(dynamic.postId)" class="text-[white] px-[15px] py-[2px] rounded-[15px] cursor-pointer bg-pink-500/20 text-pink-200 shadow-lg border border-pink-400/60" title="点赞">
            ...
            <div class="mt-2">
              ...
              <div class="mt-4 mb-2 text-center" v-if="dynamic.commentCount>0&&hasMoreComment">
              ...
              <!-- <div class="mb-2 text-center" v-if="dynamic.commentCount===0">
                <span class="text-[#6f7071]">暂没有人评论，赶紧抢占沙发吧~</span>
              </div> -->
            </div>
          </div>
        </div>
      ...
      <div class="flex-grow-0 shrink-0 basis-[28%] ml-10 mb-4 duration-[.2s] bg-gray-900/60 rounded-xl border border-gray-800 overflow-hidden shadow-lg backdrop-blur-sm w-100 p-[15px]">
        ...
        <div class="flex flex-col items-center">
          ...
          <p class="text-[#e5e7eb] text-[18px] font-[600] mt-2">{{ dynamic.author }}</p>
          <p class="text-[#bdbdbd] mt-[2px] text-[12px]">这里填introduce</p>
          <div class="flex mt-6 text-[#e5e7eb] mb-2">
            ...
            <span class="mx-14">
              <span class="mr-[2px]">{{dynamic.likeCount}}</span>
              点赞
            </span>
            <span>
...
<script setup lang="ts">
  ...
  async function getDynamicComment(){      // 获取动态评论
    ...
    if(res.success){
      hasMoreComment = res.data.hasMore;
  ...
  let throttleCommentDynamic = throttle(async function commentDynamic(dynamicId:string){      // 评论动态
    ...
    if(commentRes.success){
      ...
      commentInput.value.value = ''     // 这里等待后端响应的数据完整，以便显示新的评论
