      ...
      <div class="flex-grow flex border-b border-gray-400/20 pb-2">
        <div class="ml-4 flex flex-col">
          <div class="flex items-center">
            ...
            <span class="ml-3 text-[14px] text-[#e5e7eb]">点赞了我的评论</span>
          </div>
          <div class="mt-[6px]">
            <span class="text-[#e5e7eb] text-[13px]">{{ data.createTime }}</span>
          </div>
        </div>
        <div class="ml-auto w-[150px]">
          <p class="text-[15px] text-[#e5e7eb] line-clamp-3 hover:text-pink-400 duration-200 cursor-pointer">{{ data.targetContent }}</p>
        </div>
      </div>
    </div>
  </transition>
...
<script setup lang="ts">
  import {GetLikeMessageListDataInterface} from "@/types/message";
